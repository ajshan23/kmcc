...
export const editMembership = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    // Find and update corresponding user if exists
    ...
    if (existingUser) {
      ...
      await prismaClient.user.update({
        ...
        data: {
          ...
          areaName: areaName || existingUser.areaName,
          // Update profile if kmccPosition was based on areaName
          profile: areaName ? {
            update: {
              kmccPosition: `kmcc ${areaName} member`
            }
          } : undefined
        }
      });
