...
export const applyJobNew = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (existingApplication) {
    throw new ApiError(400, "An application with this email already exists for this job.");
  }
  ...
  try {
    ...
    res.status(201).json(
      new ApiResponse(201, application, "Job application submitted successfully.")
    );
  } catch (error) {
