...
exports.closeJob = exports.getJobApplications = exports.deleteJob = exports.editJob = exports.getAllJobsAdmin = exports.getJobByIdNew = exports.getJobById = exports.getActiveJobs = exports.applyJob = exports.applyJobNew = exports.createJob = void 0;
...
exports.applyJobNew = (0, asyncHandler_1.asyncHandler)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { jobId, fullName, email, phone } = req.body;
    // Validate authentication
    if (!req.user) {
        throw new apiHandlerHelpers_1.ApiError(401, "Authentication required");
    }
    ...
    // Convert and validate job ID
    const parsedJobId = Number(jobId);
    if (isNaN(parsedJobId)) {
        throw new apiHandlerHelpers_1.ApiError(400, "Invalid job ID.");
    }
    // Check for existing application using application's email
    const existingApplication = yield db_1.prismaClient.jobApplication.findFirst({
        where: {
            jobId: parsedJobId,
            email: email // Use the email from the application form
        },
    });
    if (existingApplication) {
        throw new apiHandlerHelpers_1.ApiError(400, "An application with this email already exists for this job.");
    }
    ...
    if (job.isClosed) {
        throw new apiHandlerHelpers_1.ApiError(400, "This job is no longer accepting applications.");
    }
    // Process and store the resume file
    try {
...
exports.getJobById = (0, asyncHandler_1.asyncHandler)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (req.user) {
        const userApplication = yield db_1.prismaClient.jobApplication.findFirst({
            where: {
                jobId,
    ...
    const formattedJob = Object.assign(Object.assign({}, job), { logo: job.logo
            ? `data:image/jpeg;base64,${Buffer.from(job.logo).toString("base64")}`
            : null, applicationCount, // Total applications for this job
        hasApplied });
    res.json(new apiHandlerHelpers_1.ApiResponse(200, formattedJob, "Job details retrieved successfully."));
}));
exports.getJobByIdNew = (0, asyncHandler_1.asyncHandler)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const jobId = Number(req.params.jobId);
    if (!jobId)
        throw new apiHandlerHelpers_1.ApiError(400, "Job ID is required.");
    ...
    if ((_a = req.user) === null || _a === void 0 ? void 0 : _a.userId) {
        // Get the user's identifying information
        const user = yield db_1.prismaClient.user.findUnique({
            where: { id: req.user.userId },
            select: { email: true, phoneNumber: true, memberId: true }
        });
        if (user) {
            // Check using all possible identifiers
            const userApplication = yield db_1.prismaClient.jobApplication.findFirst({
                where: {
                    jobId,
                    OR: [
                        { email: user.email || undefined }, // Only check if email exists
                        { phone: user.phoneNumber }, // Phone is required in User model
                        // Add other identifiers if needed
                    ].filter(condition => Object.values(condition)[0] !== undefined) // Remove undefined conditions
                },
            });
            hasApplied = !!userApplication;
        }
